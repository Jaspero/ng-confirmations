...
@Component({
    ...
    template: `
        ...
        <div class="wrapper">
            <div class="close" *ngIf="incomingData.showCloseButton" (click)="resolve({closedWithOutResolving: 'closeClick'})">
            ...
            <div class="inner">
                ...
                <p>{{incomingData.message}}</p>
                <button (click)="resolve({resolved: true})">{{incomingData.confirmText}}</button>
                <button (click)="resolve({resolved: false})">{{incomingData.declineText}}</button>
            </div>
        </div>
    `
})
